...
def main(config):
    ...
    if 'SAC' in list(config.items())[1]:
        reward_type = config['SAC']['reward_function']
    elif 'coGAIL' in list(config.items())[1]:
        reward_type = config['coGAIL']['reward_function']
    elif 'PPO' in list(config.items())[1]:
        reward_type = config['PPO']['reward_function']
    elif 'NO_ALGO' in list(config.items())[1]:
        reward_type = config['NO_ALGO']['reward_function']
    else:
...
def reward_function_distance(goal_reached, timedout, ball, goal=None):
    ...
    if goal_reached:
        ...
        reward = -get_distance_from_goal(ball, goal) / 225.0
    ...
    min_value = -1 #-5 #0
    max_value = 0 #10 #1
    return min_value + ((reward-r_min)*(max_value-min_value)/(r_max-r_min))

def reward_function(goal_reached, timedout, goal=None):
    # Construct here the mathematical reward function
    # The reward function during the train_game_number can be static, or it can depend on time,
    # the distance of the ball to the goal, etc.
    # The reward when the goal reached can also be defined as well as the penalty if the train_game_number
    # times out.
    # Once you write the function you need to do two things.
    # First, set it as an option in the `reward_function_maze` function and set the corresponding key
