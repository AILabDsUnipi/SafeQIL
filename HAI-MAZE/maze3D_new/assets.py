...
class ObjModel:
    def __init__(self, filepath):
        ...
        for name_, material in scene.materials.items():
        ...
        # In case of torus object, compute the diameter and radius of the outer circle
        if filepath.split("/")[-1].split(".")[0] == "torus":
            self.radius_outer_circle = None
            self.diameter_outer_circle = None
            self.compute_torus_diameter(filepath)
    ...
    def compute_torus_diameter(self, filepath):
        # code generated in part by chatGPT-3.5

        scene = pwf.Wavefront(filepath, collect_faces=True)

        # Extract vertices from the loaded data
        vertices = scene.vertices

        # Find the maximum and minimum x-coordinates of the vertices
        max_x = max(vertices, key=lambda vertex: vertex[0])[0]
        min_x = min(vertices, key=lambda vertex: vertex[0])[0]

        # Find the maximum and minimum y-coordinates of the vertices
        max_y = max(vertices, key=lambda vertex: vertex[1])[1]
        min_y = min(vertices, key=lambda vertex: vertex[1])[1]

        # Calculate the diameter of the outer circle
        self.diameter_outer_circle = max(max_x - min_x, max_y - min_y)

        # Calculate radius
        self.radius_outer_circle = self.diameter_outer_circle / 2.0
