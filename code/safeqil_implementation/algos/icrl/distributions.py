...
from safeqil_implementation.algos.icrl.utils import get_action_dim
...
class MultiCategoricalDistribution(Distribution):
    ...
    def proba_distribution(self, action_logits: th.Tensor) -> "MultiCategoricalDistribution":
        self.distributions = [
            Categorical(logits=split) for split in th.split(action_logits, self.action_dims, dim=1)
        ]
        return self
...
class BernoulliDistribution(Distribution):
    """
    Bernoulli's distribution for MultiBinary action spaces.

    :param action_dims: Number of binary actions
    """
...
def make_proba_distribution(action_space: gym.spaces.Space) -> Distribution:
    """
