...
# Experiment
from experiments.train_icrl_experiment import ICRLExperiment
...
from algos.icrl.plot_utils import save_constraint_net_logs_and_plot
...
def main(argv):
    ...
    assert (config['Experiment']['max_timesteps_per_game'] < config['ICRL']['n_steps'])
    ...
    icrl_agent = get_icrl_agent(config, env, seed=seed)

    # Create the experiment
    experiment = ICRLExperiment(env, icrl_agent, config, files_dir, seed)
    ...
    test_icrl_agent = get_icrl_agent(config, env, only_test=True, seed=test_seed)

    # Create the test experiment
    test_experiment = TestExperiment(env, test_icrl_agent, config, test_files_dir, test_seed)
